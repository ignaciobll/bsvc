...
def sim_write(command):
    p.stdin.write('{}\n'.format(command).encode()) # Necesita binario y salto de linea
    p.stdin.flush() # Necesario para que la entrada se haga efectiva.
...
def list_maximum_address(addrspace):
    ...
    sim_write(q)
    return sim_read()
def list_memory(addrspace, addr, lenght, words_per_line):
